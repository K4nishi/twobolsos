...
def criar_negocio(n_in: NegocioBase, session: Session = Depends(get_session), user: User = Depends(get_current_user)):
...
def get_dashboard(id: int, dias: int = 7, session: Session = Depends(get_session), user: User = Depends(get_current_user)):
    ...
    for i in range(dias - 1, -1, -1):
        ...
        dia_str = dia.isoformat() # Using isoformat for compare if transacao.data is str
...
@router.post("/join")
def join_negocio(code: str, session: Session = Depends(get_session), user: User = Depends(get_current_user)):
...
def list_members(id: int, session: Session = Depends(get_session), user: User = Depends(get_current_user)):
    n = session.get(Negocio, id)
    if n.owner_id != user.id:
         # Optionally allow members to see other members? Let's say yes but read-only
         pass
