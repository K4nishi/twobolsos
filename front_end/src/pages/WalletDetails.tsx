...
import { ArrowLeft, ArrowUpCircle, ArrowDownCircle, Trash2, Users, MoreVertical, CalendarCheck, Filter, ChevronDown, Plus, Minus } from 'lucide-react';
...
export default function WalletDetails() {
    ...
    return (
        <div className="min-h-screen pb-24 sm:pb-6 overflow-x-hidden">
            <div className="p-4 max-w-4xl mx-auto">
                ...
                <div className="grid grid-cols-3 gap-2 sm:gap-3 mb-6">
                    <div className="glass-card p-3 sm:p-4 rounded-2xl text-center">
                        <p className="text-[10px] sm:text-xs font-bold uppercase text-green-600 dark:text-green-500 mb-1">Entradas</p>
                        <p className="text-sm sm:text-lg font-bold text-zinc-900 dark:text-white truncate">R$ {kpis?.receita.toFixed(2)}</p>
                    </div>
                    <div className="glass-card p-3 sm:p-4 rounded-2xl text-center">
                        <p className="text-[10px] sm:text-xs font-bold uppercase text-red-600 dark:text-red-500 mb-1">Saídas</p>
                        <p className="text-sm sm:text-lg font-bold text-zinc-900 dark:text-white truncate">R$ {kpis?.despesa.toFixed(2)}</p>
                    </div>
                    <div className="glass-card p-3 sm:p-4 rounded-2xl text-center bg-zinc-900/5">
                        <p className="text-[10px] sm:text-xs font-bold uppercase text-blue-600 dark:text-blue-500 mb-1">Saldo</p>
                        <p className="text-sm sm:text-lg font-bold text-zinc-900 dark:text-white truncate">R$ {kpis?.saldo.toFixed(2)}</p>
                    </div>
                </div>

                {/* Action Buttons - Desktop only (mobile has fixed footer) */}
                        ...
                        {filteredTransactions.map(t => (
                            <div key={t.id} className="flex justify-between items-center p-3 hover:bg-zinc-50 dark:hover:bg-white/5 rounded-xl transition-colors group">
                                <div className="flex items-center gap-3 min-w-0 flex-1">
                                    <div className={`w-10 h-10 rounded-full flex items-center justify-center flex-shrink-0 ${t.tipo === 'receita' ? 'bg-green-100 dark:bg-green-500/20 text-green-600 dark:text-green-500' : 'bg-red-100 dark:bg-red-500/20 text-red-600 dark:text-red-500'}`}>
                                        {t.tipo === 'receita' ? <ArrowUpCircle size={20} /> : <ArrowDownCircle size={20} />}
                                    </div>
                                    <div className="min-w-0 flex-1">
                                        <p className="font-bold text-zinc-900 dark:text-white text-sm truncate">{t.descricao}</p>
                                        <p className="text-xs text-zinc-500 flex items-center gap-1 flex-wrap">
                                            ...
                                            <span className="text-zinc-400 truncate max-w-[60px] sm:max-w-none">{t.created_by_name}</span>
                                ...
                                <div className="text-right flex items-center gap-2 sm:gap-4 flex-shrink-0">
                                    <span className={`font-bold text-sm sm:text-base ${t.tipo === 'receita' ? 'text-green-600 dark:text-green-500' : 'text-red-600 dark:text-red-500'}`}>
                                        {t.tipo === 'receita' ? '+' : '-'} R$ {t.valor.toFixed(2)}
                                    </span>
                                    <button onClick={() => deleteTransaction(t.id)} className="text-zinc-300 hover:text-red-500 transition-colors opacity-0 group-hover:opacity-100 p-1">
                                        <Trash2 size={16} />
                                    </button>
                                </div>
                            </div>
            ...
            <div className="fixed bottom-0 left-0 right-0 sm:hidden bg-white/80 dark:bg-zinc-900/80 backdrop-blur-lg border-t border-zinc-200 dark:border-zinc-800 p-3 z-30 safe-area-pb">
                <div className="grid grid-cols-2 gap-3 max-w-md mx-auto">
                    <button
                        onClick={() => setTransType('receita')}
                        className="bg-gradient-to-r from-green-600 to-green-500 text-white py-3.5 px-4 rounded-xl shadow-lg shadow-green-500/30 active:scale-95 transition-all flex justify-center items-center gap-2 font-bold text-sm"
                    >
                        <Plus size={20} className="stroke-[3]" />
                        <span>Entrada</span>
                    </button>
                    <button
                        onClick={() => setTransType('despesa')}
                        className="bg-gradient-to-r from-red-600 to-red-500 text-white py-3.5 px-4 rounded-xl shadow-lg shadow-red-500/30 active:scale-95 transition-all flex justify-center items-center gap-2 font-bold text-sm"
                    >
